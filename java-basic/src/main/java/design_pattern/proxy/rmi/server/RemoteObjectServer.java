...
public class RemoteObjectServer {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      // => 첫번째 파라미터 : 원격 객체
      // => 두번째 파라미터 : 포트번호, 0으로 지정하면 임의로 유효한 포트번호 생성
      // => 이렇게 RMI 기술을 사용하면 개발자가 skeleton(서버측 ORB)이나 stub(클라이언트측 ORB)를 생성하지 않아도 된다.
      ...
      // => rebind() : 해당 이름으로 stub 객체를 등록한다.
      //                    만약 그 이름으로 등록된 stub이 있다면 기존에 등록된 stub 객체를 대체한다.
      // => bind() : 해당 이름으로 stub 객체를 등록한다.
      //                    만약 그 이름으로 등록된 stub이 있다면 예외가 발생한다.
