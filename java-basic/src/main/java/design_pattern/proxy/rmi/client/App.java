...
// => RMI Registry 서버에 등록된 stub을 받아서 원격의 CalculatorImpl 객체를 사용한다.
...
public class App {
  public static void main(String[] args) throws Exception {
    ...
    // 1) RMI Registry 서버에서 stub 객체를 받기 위해 사용할 도구를 준비한다.
    Registry registry = LocateRegistry.getRegistry("localhost");
    
    // 2) 이 도구를 사용하여 RMI Registry 서버에 등록된 stub을 등록된 이름으로 찾는다.
    // => lookup() : RMI Registry 서버에서 해당 이름으로 등록된 stub을 찾아서 리턴한다.
