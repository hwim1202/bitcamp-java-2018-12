// URL에서 값 추출하기 - @PathVariable
package bitcamp.app2;

import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseBody;

@Controller
@RequestMapping("/c02_1")
public class Controller02_1 {

  // http://localhost:8080/java-spring-webmvc/app2/c02_1?name=kim&age=20
  @GetMapping
  @ResponseBody
  public String handler1(String name, int age) {
    // 클라이언트에서 값을 받는 일반적인 방법
    // Query String --> URL다음에 '?변수=값&변수=값' 형태로 받는다
    // Query String의 값을 request handler에서 받으려면 아규먼트를 선언한다
    // 아규먼트 앞에 @RequestParam을 붙이면 된다. 요청 파라미터의 이름과 같다면 @RequestParam 생략가능
    return String.format("name=%s, age=%d,", name, age);
  }

  // http://localhost:8080/java-spring-webmvc/app2/c02_1/kim/20
  @GetMapping("{name}_{age}")
  @ResponseBody
  public String handler2(/*@PathVariable("name")String name, @PathVariable("age")int age,*/
      @PathVariable String name, @PathVariable int age
      ) {
    // URL에 '/' 형식으로 포함된 값을 받으려면 다음의 문법으로 선언해야한다
    // .../{변수명}/{변수명}
    // @PathVariable(변수명) String 아규먼트
    return String.format("name=%s, age=%d,", name, age);
  }

}